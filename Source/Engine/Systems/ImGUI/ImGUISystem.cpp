...
public:
	...
	{
		// all resources internal and managed by render pass
		// so they are ready
		return true;
	}
...
protected:
	...
	void BeginPass(Graphics& aGraphics) override final
	{
...

		{
			...
			myUniformBlock->SetUniform(0, orthoProj);
		}
		...
		myModel->GetResource().Get<Model>()->Update(uploadDesc);
...
void ImGUISystem::Init()
{
	...
	ImGUIRenderPass* imguiRenderPass = new ImGUIRenderPass(imGUIPipeline, fontAtlas, *myGame.GetGraphics());
	myGame.GetGraphics()->AddRenderPass(imguiRenderPass);
}
