...
Grid::Grid(glm::ivec3 start, glm::ivec3 end, glm::vec3 cellSize)
	: start(start)
	, end(end)
	, cellSize(cellSize)
{
...
void Grid::Add(GameObject *go, uint32_t threadId)
{
	...
	glm::vec3 loc = go->GetTransform()->GetPos();
	...
	glm::vec3 AABBStart = loc - glm::vec3(radius, radius, radius);
	glm::vec3 AABBEnd = loc + glm::vec3(radius, radius, radius);

	// constraining coords to edges, if needed
	AABBStart = glm::clamp(AABBStart, start, end);
	AABBEnd = glm::clamp(AABBEnd, start, end);
	...
	glm::vec3 coordsStart = glm::vec3(
		glm::clamp(floor(AABBStart.x / cellSize.x), 0.f, width - 1.f),
		glm::clamp(floor(AABBStart.y / cellSize.y), 0.f, height - 1.f),
		glm::clamp(floor(AABBStart.z / cellSize.z), 0.f, depth - 1.f)
	);
	glm::vec3 coordsEnd = glm::vec3(
		glm::clamp(floor(AABBEnd.x / cellSize.x), 0.f, width - 1.f),
		glm::clamp(floor(AABBEnd.y / cellSize.y), 0.f, height - 1.f),
		glm::clamp(floor(AABBEnd.z / cellSize.z), 0.f, depth - 1.f)
	);
